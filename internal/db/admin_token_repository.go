...
func (r *AdminTokenRepository) Create(ctx context.Context, token domain.AdminToken) error {
	_, err := r.db.conn.ExecContext(ctx, `INSERT INTO admin_tokens (token_hash, display_name, created_at, last_used_at, revoked_at) VALUES (?, ?, ?, ?, ?)`,
...
func (r *AdminTokenRepository) FindByHash(ctx context.Context, tokenHash string) (domain.AdminToken, error) {
	...
	err := r.db.conn.QueryRowContext(ctx, `SELECT token_hash, display_name, created_at, last_used_at, revoked_at FROM admin_tokens WHERE token_hash = ?`, tokenHash).
...
func (r *AdminTokenRepository) MarkUsed(ctx context.Context, tokenHash string, usedAt time.Time) error {
	return mapRowsAffected(r.db.conn.ExecContext(ctx, `UPDATE admin_tokens SET last_used_at = ? WHERE token_hash = ?`, formatTime(usedAt), tokenHash))
}
